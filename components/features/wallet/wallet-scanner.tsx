    ...
    <div className="flex flex-col gap-2.5">
      <div className="relative">
        ...
        >
          <span className="flex items-center gap-2">
            <span className="size-2 rounded-full bg-accent shadow-[0_0_8px_rgba(251,191,36,0.5)]" />
            {network}
          </span>
            ...
            {networks.map((n) => (
              <li key={n}>
                <button
                  ...
                  className={`flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm transition-colors hover:bg-accent/[0.06] ${
                    n === network ? 'text-accent' : 'text-foreground'
                  }`}>
                <span className="size-2 rounded-full bg-accent shadow-[0_0_8px_rgba(251,191,36,0.5)]" />
                  {n}
                </button>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-primary/30 bg-black/60 py-2.5 pl-9 pr-3 font-mono text-sm text-foreground placeholder:text-muted-foreground focus:border-accent focus:outline-none focus:shadow-[0_0_15px_rgba(251,191,36,0.2)] transition-all"
        />
      </div>
